...
1. Execute `wget https://github.com/minprog/cs50x/raw/2020/survey/survey.zip` to download a (compressed) ZIP file with this problem's distribution.
...
- `cd module10/survey`
- `zip -r survey.zip .` (don't forget the `.` for current directory!)

The latter command will compress all files in the current (`.`) directory and recursively (`-r`) all files in subdirectories into a file called `survey.zip`. This way, all files including templates will be added to the zipfile.
