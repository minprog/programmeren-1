...
1. Execute `cd ~/problems` to ensure that you're in `~/problems` (i.e., a directory called `problems` that's in your home directory, aka `~`). If you haven't yet created that directory, create it now (remember how?).
1. Execute `wget https://github.com/minprog/cs50x/raw/2020/bleep/bleep.zip` to download a (compressed) ZIP file with this problem's distribution.
