...
def cracks_andi():
    ...
    check50.run("./crack 50.jPgLzVirkc").stdout("hi", timeout=50).exit(0)

@check50.check(compiles)
def cracks_jason():
    """cracks jason's password: 50YHuxoCN9Jkc"""
    check50.run("./crack 50YHuxoCN9Jkc").stdout("JH", timeout=50).exit(0)
...
def cracks_mzlatkova():
    ...
    check50.run("./crack 50CPlMDLT06yY").stdout("ha", timeout=50).exit(0)
