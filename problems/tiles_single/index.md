...
`tiles.c` defines the **board**, which is in our program a data structure (a 2-D array, in this case) to store the game "state" as the game progresses.
...
Read over the code and comments in `tiles.c` and then answer the questions below for yourself. You don't have to hand these in, but you'll need the answers to write the code. No worries if you're not quite sure how `fprintf` or `fflush` work; we're simply using those to automate some testing.
...
Implement Tiles, per the comments in `tiles.c` and the information from this specification.

## Testing

To test your implementation of `tiles`, you can certainly try playing it. Be sure that you (and we) cannot crash your program, as by providing bogus tile numbers.

<details markdown="1"><summary markdown="span">Spoilers: here are all moves to win a 3x3 board</summary>
3
4
1
2
