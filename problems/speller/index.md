...
In `cs50.dev` you can also download the distribution code directly from the terminal:
    ...
    $ cd speller
    $ ls
    answers.md  dictionaries/  dictionary.c  dictionary.h  keys/  Makefile  speller.c  texts/

### Understanding

Theoretically, on input of size _n_, an algorithm with a running time of _n_ is asymptotically equivalent, in terms of _O_, to an algorithm with a running time of 2_n_. In the real world, though, the fact of the matter is that the latter feels twice as slow as the former.
...
So that you can test your implementation of `speller`, we've also provided you with a whole bunch of texts, among them the script from _La La Land_, the text of the Affordable Care Act, three million bytes from Tolstoy, some excerpts from _The Federalist Papers_ and Shakespeare, the entirety of the King James V Bible and the Koran, and more. So that you know what to expect, open and skim each of those files, all of which are in a directory called `texts` within your `speller` directory.
...
Open up `answers.md` and answer each of the questions therein.
...
> Store the dictionary in a **hash table**: an array of buckets, where a hash function decides which bucket a word belongs in, and each bucket holds the words that hashed to it. Designing a good hash function and picking a sensible number of buckets is exactly the part you get to think about.

> Would you rather use a **trie**? That is the star assignment [Speller Challenge](/problems/speller/challenge), where you build a trie version next to this one and compare how fast the two are. Do the hash table first.
...
![embed](https://www.youtube.com/embed/u9-1U1Rgo1o?list=PLhQjrBD2T382vvokQIExRCKZq-q8PzSVz)
